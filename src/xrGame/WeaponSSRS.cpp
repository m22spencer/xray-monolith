		...
		if (GetState() == eIdle && getRocketCount() && iAmmoElapsed)
		{
			...
			if (IsGameTypeSingle() && IsZoomed() && smart_cast<CActor*>(H_Parent()))
			{
				...
				if (HasPick)
				{
					...
					/*#ifdef		DEBUG
									DBG_OpenCashedDraw();
									DBG_DrawLine(p1,Fvector().add(p1,d),D3DCOLOR_XsrB(255,0,0));
					#endif*/
#ifdef CROCKETLAUNCHER_CHANGE
...
#endif
					/*#ifdef DEBUG
									if(canfire0>0)DBG_DrawLine(p1,Fvector().add(p1,res[0]),D3DCOLOR_XsrB(0,255,0));
									if(canfire0>1)DBG_DrawLine(p1,Fvector().add(p1,res[1]),D3DCOLOR_XsrB(0,0,255));
